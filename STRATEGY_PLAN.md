...
对 5 分钟二元盘，单边趋势里的价格斜率比静态价格更重要。动量信号主要用于中前段更快识别方向，不用于最后阶段追单。
...
- BTC 连续上冲时，提高 `p_up`。
- BTC 连续下压时，提高 `p_down`。
- 价格接近 strike 时，可以适度提高动量权重，但不能因为接近结算就自动加权。
- 离 strike 很远时，动量权重降低，避免噪声过度影响。
- 进入最后风险区或 `ENDGAME_PULL_SECS` 后，不用动量信号提高报价，只撤单、等待或保持停止新报价。
...
- 不在最后阶段因为动量或订单流信号追单；`ENDGAME_PULL_SECS` 仍然优先生效。
